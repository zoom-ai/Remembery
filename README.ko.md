...
### 🔒 안전한 멀티 유저 아키텍처 및 철저한 데이터 격리
*   **JWT 기반 보안 인증 세션**: **로그인** 및 **회원가입** 폼을 지원하고, 글로벌 React `AuthContext`와 브라우저 `localStorage` 세션을 연동하였습니다. Axios 요청 인터셉터를 통해 모든 API 호출 헤더에 JWT 토큰(`Authorization: Bearer {token}`)을 자동으로 주입합니다.
*   **자동 SQLite 스키마 마이그레이션 & 데이터 백필**: 서버 실행 시 자동으로 DB 테이블을 검사(`PRAGMA table_info`)하여 필요한 `user_id` 외래키 컬럼들을 추가하고, 기존에 적재된 아카이브 데이터들을 기존 사용자(User ID 1)에게 유실 없이 자동 백필(Backfill) 연동해 줍니다.
*   **완벽한 멀티테넌트 데이터 보호**: 모든 카테고리 조회, 파일 업로드, AI 가상 미술관 큐레이팅, AI 도슨트 RAG 의미론적 검색 범위 및 타임라인 연혁 조회 쿼리를 로그인한 유저 ID(`user_id == current_user.id`)로 강제 구속하여 다른 사용자의 아카이브에 접근하는 것을 완벽히 차단합니다.
*   **격리된 타임라인 및 프로필 연혁 관리**: 로그인한 사용자 세션에 안전하게 범위가 제한된 수동 타임라인 이벤트 추가(`POST /api/users/timeline`) 및 인덱스 기반 삭제(`DELETE /api/users/timeline/{index}`)를 지원합니다.
*   **자동 이력서-타임라인 동기화**: 일괄 저장(`POST /api/archive/batch`)을 통해 이력서를 가져올 때, 파싱된 항목들이 고유한 카테고리 이모지(`🎓`, `💼`, `🏆`, `🌱`)를 사용하여 시간 순서대로 타임라인 이벤트에 자동으로 매핑되고 사용자의 격리된 프로필 연혁(`User.timeline_json`)에 직접 추가됩니다.
...
- [x] **AI 이력서 타임라인 임포터** 지원 완료 (텍스트 붙여넣기 / PDF / DOCX / TXT 파일 분석 및 고품격 편집 그리드 제공)
- [x] **AI 역량 평가 레이더 리포트** 개발 완료 (Chart.js 기반 5대 전문성 지표 시각화 및 맞춤형 AI 도슨트 코멘터리 연동)
- [x] **안전한 멀티 유저 아키텍처 구축** 완료 (FastAPI + JWT 기반 보안 데이터 격리, React Context 세션 관리 및 Axios 인터셉터 통합)
- [x] **격리된 타임라인 및 이력서 연혁 자동 동기화** 개발 완료 (수동 연혁 추가 및 삭제, 이력서 임포트 시 자동 이모지 매핑 동기화)
